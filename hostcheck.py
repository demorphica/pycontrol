...
def growler(message):
        try:
                ...
                syslog.syslog(syslog.LOG_ALERT, "Error in notification subsystem")
                return 0
...
class Host:
    ...
    def pingcheck(self):
        ...
        try:
            ...
            growler("ICMP echo request test check for "+self.hostname+"failed unexpectedly")
            return 3
        if int(packetloss) == 100:
            ...
            growler("ICMP echo request test resulted in 100% packets lost. Host "+self.hostname+" is down")
            return 0
        elif int(packetloss) == 0:
            syslog.syslog(syslog.LOG_ALERT, "ICMP echo request test succeded Host "+self.hostname+" is up")
            growler("ICMP echo request test succeded Host "+self.hostname+" is up")
            return 1
    ...
    def sleep(self):
        try:
            ...
            growler("Could not determine success for sending sleep signal to host "+self.hostname)
            return(0)
    ...
    def wake(self):
        try:
            ...
            growler("Sent WoL packet to host"+self.hostname)
            return(1)
        except ValueError:
            syslog.syslog(syslog.LOG_ALERT, "Error sending  WoL packet to host"+self.hostname)
            growler("Error sending  WoL packet to host"+self.hostname)
            return(0)
    ...
    def check(self):
        ...
        growler("Average load for all test runs is "+str(avgloadlastduration)+"%")
...
def main():
    ...
    growler("Checking host "+host.username+" for load status started")
    ...
    if ishostalive == 3:
        ...
        status=host.check()
        if status > 1.0:
            growler("Host "+host.hostname+" is Working...")
            sys.exit()
        elif status <= 1.0:
            growler("Host "+host.username+" has been Idle for "+str(host.duration)+" seconds now")
            syslog.syslog(syslog.LOG_ALERT, "Host "+host.username+" has been Idle for "+str(host.duration)+" seconds now")
            syslog.syslog(syslog.LOG_ALERT, "There is no demand for host "+host.hostname+". Sending sleep signal to host "+host.hostname)
            host.sleep()
            growler("There is no demand for host "+host.hostname+". Sending sleep signal to host "+host.hostname)
            sys.exit()
        else:
            syslog.syslog(syslog.LOG_ALERT, "Could not reliably determine load for host "+host.hostname)
            sys.exit()
